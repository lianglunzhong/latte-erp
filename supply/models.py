...
#     )
    ...
    def save(self, *args, **kwargs):
        ...
        if self.status==4:
            PurchaseOrderItem.objects.filter(purchaseorder_id=self.id).update(status=0)
        if self.type == 2:
...
class PurchaseOrderItem(models.Model):
    ...
    created = models.DateTimeField(auto_now_add=True, verbose_name=u"新增时间")
    updated = models.DateTimeField(auto_now=True, verbose_name=u"修改时间")
    reportmiss_date = models.DateField(blank=True, null=True, verbose_name=u"报缺时间")
   ...
   #    unique_together = ('purchase_order', 'item',)
    ...
    def update_purchaseorder_status(self):
        ...
        poi_status = PurchaseOrderItem.objects.filter(purchaseorder=self.purchaseorder).exclude(status=0).values_list('in_status', flat=True)
        poi_status2 = list(set(poi_status))
        print poi_status2

        if 2 in poi_status2 and len(poi_status2) == 1:
            PurchaseOrder.objects.filter(id=self.purchaseorder_id).update(status=2)
        elif 1 in poi_status2:
    ...
    def update_purchaseorder_cost(self):
        ...
        pois = PurchaseOrderItem.objects.filter(purchaseorder=self.purchaseorder).exclude(status=0)
        total = 0
        for poi in pois:
    ...
    def save(self, *args, **kwargs):
        ...
        if self.status==1:
            ...
            elif self.real_qty<self.qty:
                self.action_status=1
            else:
                self.action_status=2

            if self.depotinlog_qty==0:
                self.in_status=0
            elif self.depotinlog_qty<self.qty:
                self.in_status=1
            else:
                self.in_status=2

        if self.status==2:
            self.reportmiss_date = self.updated
    ...
    def save(self, *args, **kwargs):
        if not self.deleted:
            ...
            if depotinlog_qty_added != 0:
                ...
                self.purchaseorderitem.update_purchaseorder_status()
...
class PurchaseRequestItem(models.Model):
    purchaserequest = models.ForeignKey(PurchaseRequest, verbose_name=u"采购方案", related_name='purchaserequest')
