...
class SupplyImageInline(admin.TabularInline):
    ...
    def thumb(self, obj):
        ...
        # return output
        return obj.product.get_image_thumb()
...
class PurchaseOrderItemInline(admin.TabularInline):
    ...
    def thumb(self, obj):
        ...
        #     url = "http://placehold.it/100x100"
        #     output =u'<img src="%s" width="100px"/>' % (url)
        output = u'%s' % (obj.item.product.get_image_thumb())
        return output
    ...
    fields = ['id','item', 'thumb', 'purchaseorder', 'cost', 'qty','real_qty','depotinlog_qty','note','status', 'action_status', 'estimated_date', 'in_status', 'purchase_request_filed' ]
...
class PurchaseOrderItemAdmin(NoDeleteActionModelAdmin):
 ...
 # editing an existing object 区分add和change界面，change设置只读
    def get_readonly_fields(self, request, obj=None):
        if obj:
            return self.readonly_fields + ('purchaseorder', 'item', 'qty', 'real_qty','depotinlog_qty','depot','supplier')
    ...
    def thumb(self, obj):
        ...
        output = obj.item.product.get_image_thumb()
        return output
    ...
    save_as = True
    save_on_top = True
    list_filter = ('action_status',PurchaseOrderItemManagerFilter,'in_status','purchaseorder__status','status')
    list_display = ('id','purchase_item_id','thumb','action_status','in_status','status','note','supplier','supplier_sku','manager','item_qty', 'qty','input_qty','real_qty','depotinlog_qty', 'cost','created', 'estimated_date')
 ...
 #处理历史的采购物品成本
    ...
    def export_purchaseorderitem(modeladmin, request, queryset):
        ...
        writer = csv.writer(response)
        writer.writerow(["采购单ID","采购单状态","采购条目状态","采购条目ID","sku","颜色","尺码","item数量","对单数量","实际入库数量","未对单数量","采购价","供应商","供应商SKU","备注","采购单生成时间","负责人","采购link","处理状态","处理状态","Model","choies_sku"])
        for query in queryset:
