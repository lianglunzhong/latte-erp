...
#class ProductAdmin(admin.ModelAdmin):
    ...
    save_as = True
    save_on_top = True
    search_fields = ['sku','choies_sku', 'name', 'note',]
    list_display = ('id', 'sku_view','choies_sku', 'cost', 'weight', 'name', 'thumb', 'status', 'category','adder', 'created')
...
class ItemAdmin(NoDeleteActionModelAdmin):
    ...
    list_display = ('id', 'p_link', 'sku','thumb','key', 'cost', 'weight', 'status', )
...
class ProductRequestProductInline(admin.TabularInline):
    ...
    def show_extra_filed(self,obj):
        ...
        # else:
        #     url = "http://placehold.it/100x100"
        url = obj.product.get_image_thumb()
        str_image = u'%s <br/> %s <a href="%s" target="_blank">产品详情</a>' % (obj.product.name,url,obj.product.get_admin_url())
        return str_image
